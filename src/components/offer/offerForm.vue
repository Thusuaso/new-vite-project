<template>
        ...
        <div class="row m-auto mt-4">
          <div class="col">
            <span class="p-float-label">
              <AutoComplete
                id="shopper"
                v-model="selectedShopper"
                dropdown
                :suggestions="filteredOfferShopperList"
                optionLabel="musteriAdi"
                @complete="searchOfferShopper($event)"
                @item-select="offerShopperSelected($event)"
                @change="offerShopperChange($event)"
              />
          ...
          <div class="col">
            <span class="p-float-label">
              <Dropdown
                id="source"
                v-model="selectedOfferPlace"
                :options="offerPlaces"
                optionLabel="place"
                class="w-100"
                @change="offerPlaceSelected($event)"
              />
        ...
        <div class="row m-auto mt-4">
          <div class="col">
            <span class="p-float-label">
              <AutoComplete
                ...
                dropdown
                :suggestions="filteredOfferCountryList"
                optionLabel="ulkeAdi"
                @complete="searchOfferCountry($event)"
                @item-select="offerCountrySelected($event)"
              />
          ...
          <div class="col">
            <span class="p-float-label">
              <Dropdown
                id="source"
                v-model="selectedOfferPriority"
                :options="offerPriorities"
                optionLabel="priority"
                class="w-100"
                @change="offerPriorityChange($event)"
              />
        ...
        <TabView>
          <TabPanel header="Teklif Açıklama">
            <textarea
              ...
              v-model="getOfferModelList.aciklama"
            ></textarea>
            ...
            <div class="row m-auto mt-3">
              ...
              <div class="col-3">
                <a :href="offerFileLink">
                  <button
                    type="button"
                    class="pi pi-download w-25 h-100 btn btn-secondary"
                    :disabled="offerFileDownloadDisable"
                  ></button>
                </a>
              </div>
              <div class="col-3">
                <button
                  type="button"
                  class="pi pi-trash w-25 h-100 btn btn-danger"
                  @click="offerFileDelete"
                  :disabled="offerFileDeleteDisable"
                ></button>
              </div>
            </div>
          </TabPanel>
            ...
            <div class="row m-auto mt-3">
              <div class="col">
                <button
                  ...
                  @click="offer_proforma_form = true"
                  :disabled="offerProformaDisabled"
                >
                  Proforma
                </button>
              </div>
              <div class="col">
                <button
                  ...
                  @click="offer_sample_form = true"
                  :disabled="offerSampleDisabled"
                >
                  Numune
                </button>
              </div>
          ...
          <div class="col">
            <div class="form-check">
              <input
                ...
                value=""
                id="flexCheckDefault"
                v-model="getOfferModelList.takipEt"
              />
              <label class="form-check-label" for="flexCheckDefault"> Takip Et </label>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="row m-auto mt-3">
      <div class="col">
        <span class="p-float-label">
          <InputText
            id="username"
            v-model="getOfferModelList.company"
            @input="selectedShopper.company = $event.target.value"
          />
      ...
      <div class="col">
        <span class="p-float-label">
          <InputText
            id="username"
            v-model="getOfferModelList.email"
            @input="selectedShopper.email = $event.target.value"
          />
      ...
      <div class="col">
        <span class="p-float-label">
          <InputText
            id="username"
            v-model="getOfferModelList.adress"
            @input="selectedShopper.adress = $event.target.value"
          />
    ...
    <TabView>
      ...
      <TabPanel header="Hatırlatma Belge">
        ...
        <a :href="offerFileLink">
          <button
            type="button"
            class="pi pi-download w-100 h-100 btn btn-secondary mb-3"
            :disabled="offerFileDownloadDisable"
          ></button>
        </a>
        <button
          type="button"
          class="pi pi-trash w-100 h-100 btn btn-danger mb-3"
          @click="offerFileDelete"
          :disabled="offerFileDeleteDisable"
        ></button>
      </TabPanel>
  ...
  <Dialog
    v-model:visible="offer_proforma_form"
    header=""
    modal
    :style="{ width: '100vw' }"
  >
...
import offerProformaForm from "../../components/offer/offerProformaForm.vue";
import offerSampleForm from "../../components/offer/offerSampleForm.vue";
import offerProductForm from "../../components/offer/offerProductForm.vue";
export default {
  computed: {
  ...
  methods: {
    ...
    offerCreatedProcess() {
      ...
      this.selectedSource = this.sources.find(
        (x) => x.source == this.getOfferModelList.kaynakYeri
      );
      this.selectedShopper = this.getOfferShopperList.find(
        (x) => x.id == this.getOfferModelList.musteriId
      );
      this.selectedOfferPlace = this.offerPlaces.find(
        (x) => x.place == this.getOfferModelList.teklifYeri
      );
      this.selectedCountry = this.getOfferCountryList.find(
        (x) => x.id == this.selectedShopper.ulkeId
      );
      this.selectedOfferPriority = this.offerPriorities.find(
        (x) => x.priority == this.getOfferModelList.teklifOncelik
      );
      this.phone = this.getOfferModelList.phone;
      if (
        this.getOfferModelList.hatirlatmaTarihi == null ||
        this.getOfferModelList.hatirlatmaTarihi == "NaN/NaN/NaN" ||
        this.getOfferModelList.hatirlatmaTarihi == "" ||
        this.getOfferModelList.hatirlatmaTarihi == " " ||
        this.getOfferModelList.hatirlatmaTarihi == undefined
      ) {
        this.r_date = null;
      } else {
        this.r_date = localDateService.getStringDate(
          this.getOfferModelList.hatirlatmaTarihi
        );
      }
    ...
    save() {
      ...
      this.getOfferModelList.numuneHatirlatmaTarihi = this.dateNullControl(
        this.getOfferModelList.numuneHatirlatmaTarihi
      );
      this.getOfferModelList.numuneGirisTarihi = this.dateNullControl(
        this.getOfferModelList.numuneGirisTarihi
      );
      this.getOfferModelList.hatirlatmaTarihi = this.dateNullControl(
        this.getOfferModelList.hatirlatmaTarihi
      );
      ...
      offerService.save(data).then((data) => {
        if (data.status) {
          ...
          socket.socketIO.emit(
            "offer_detail_load_list_emit",
            this.getOfferRepresentativeId
          );
    ...
    offerShopperChange(event) {
      ...
      if (Object.getPrototypeOf(event.value) === String.prototype) {
        ...
        customer = this.getOfferShopperList.find(
          (x) => x.musteriAdi == event.value.musteriAdi
        );
      }
      if (!customer) {
