        ...
        <div class="row m-auto mt-3">
          <div class="col">
            ...
            <span style="font-size: 13px; text-decoration: underline; color: red">
              Açıklama eklemek istediğiniz ürünün kalem numarası ile birlikte açıklamayı
              yazınız .
            </span>
            <InputText
              type="text"
    ...
    <div class="row m-auto mt-3">
      <div class="col" style="margin-left: 65px">
        <button
          ...
          style="margin: 20px; height: 50px; width: 150px; background-color: green"
        >
          ISF-Excel
        </button>
      </div>
    ...
    <div class="m-auto mt-3">
      <div class="mb-3">
        <button
          icon="fas fa-file-excel"
          @click="excel_cikti_click"
          style="height: 50px; width: 100%; background-color: green"
        >
          ISF-Excel
        </button>
      </div>
...
export default {
  ...
  mounted() {
    ...
    productionsService.getTedarikciUrunAyrintiListYukle(this.po).then((data) => {
  ...
  methods: {
    ...
    download() {
      ...
      var dd = {
        content: [
          ...
          { text: " ", fontSize: 7 },
          {
            text: "Denizli " + this.teslimTur.tur + " : " + this.formatPrice(total),
            fontSize: 9,
            margin: [350, 40, 20, 30],
          },
          { text: " ", fontSize: 7 },
      ...
      if (event.size > 1000000) {
        ...
        if (!this.urunList) {
          this.$toast.add({
            severity: "error",
            summary: "ISF",
            detail: "Lütfen tedarikçi seçiniz.",
            life: 3000,
          });
        ...
        if (this.is_two_isf_form == false) {
            ...
            productionsService.setIcSiparisDosyaKayit(bilgi).then((veri) => {
              if (veri.Status) {
                this.$toast.add({
                  severity: "success",
                  summary: "ISF",
                  detail: "ISF başarıyla kaydedildi",
                  life: 3000,
                });
                ...
                this.$toast.add({
                  severity: "error",
                  summary: "ISF",
                  detail: "ISF kaydedilemedi, Lütfen tekrar deneyiniz.",
                  life: 3000,
                });
              }
            });
          ...
          const evrak = this.tedarikci.tedarikciadi + "-" + this.po + " -2" + ".pdf";
          fileService.faturaDosyaGonder(event.files[0], 3, evrak).then((data) => {
            console.log(data);
            const bilgi = {
    ...
    teslimTurDegisim() {
      if (
        ...
        this.tedarikci == null ||
        this.faturaTur1.length == 0 ||
        this.teslimTur.length == 0
      ) {
        this.isISFForm = true;
      } else {
